...
type RPCClient struct {
	*cttrpcclient.Client
	connConfig        *cttrpcclient.ConnConfig // Work around unexported field
...
func NewRPCClient(chainParams *chaincfg.Params, connect, user, pass string, certs []byte,
	...
	client := &RPCClient{
		connConfig: &cttrpcclient.ConnConfig{
			Host:                 connect,
			Endpoint:             "ws",
			User:                 user,
			Pass:                 pass,
	...
	ntfnCallbacks := &cttrpcclient.NotificationHandlers{
	...
	rpcClient, err := cttrpcclient.New(client.connConfig, ntfnCallbacks)
	if err != nil {
		return nil, err
	}
...
// cttrpcclient callbacks, which isn't very Go-like and doesn't allow
...
// here since cttrpcclient doesn't parse this nicely for us.
...
func (c *RPCClient) onRecvTx(tx *cttutil.Tx, block *btcjson.BlockDetails) {
...
func (c *RPCClient) onRedeemingTx(tx *cttutil.Tx, block *btcjson.BlockDetails) {
...
// POSTClient creates the equivalent HTTP POST cttrpcclient.Client.
func (c *RPCClient) POSTClient() (*cttrpcclient.Client, error) {
	configCopy := *c.connConfig
	configCopy.HTTPPostMode = true
	return cttrpcclient.New(&configCopy, nil)
}
