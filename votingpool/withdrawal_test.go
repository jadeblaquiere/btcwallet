...
import (
	...
	vp "github.com/jadeblaquiere/cttwallet/votingpool"
)
...
func TestStartWithdrawal(t *testing.T) {
	...
	checkWithdrawalOutputs(t, status, map[string]cttutil.Amount{address1: 4e6, address2: 1e6})

	if status.Fees() != cttutil.Amount(1e3) {
		t.Fatalf("Wrong amount for fees; got %v, want %v", status.Fees(), cttutil.Amount(1e3))
	}
...
func checkWithdrawalOutputs(
	t *testing.T, wStatus *vp.WithdrawalStatus, amounts map[string]cttutil.Amount) {
