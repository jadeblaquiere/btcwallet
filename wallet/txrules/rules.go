...
const DefaultRelayFeePerKb cttutil.Amount = 1e3

// IsDustAmount determines whether a transaction output value and script length would
// cause the output to be considered dust.  Transactions with dust outputs are
// not standard and are rejected by mempools with default policies.
func IsDustAmount(amount cttutil.Amount, scriptSize int, relayFeePerKb cttutil.Amount) bool {
...
func IsDustOutput(output *wire.TxOut, relayFeePerKb cttutil.Amount) bool {
	...
	return IsDustAmount(cttutil.Amount(output.Value), len(output.PkScript),
		relayFeePerKb)
}
...
// output.
func CheckOutput(output *wire.TxOut, relayFeePerKb cttutil.Amount) error {
	if output.Value < 0 {
		return ErrAmountNegative
	}
	if output.Value > cttutil.MaxSatoshi {
...
func FeeForSerializeSize(relayFeePerKb cttutil.Amount, txSerializeSize int) cttutil.Amount {
	fee := relayFeePerKb * cttutil.Amount(txSerializeSize) / 1000

	if fee == 0 && relayFeePerKb > 0 {
		fee = relayFeePerKb
	}

	if fee < 0 || fee > cttutil.MaxSatoshi {
		fee = cttutil.MaxSatoshi
	}
